...
list_keywords <- c("maternal imprint", "sex as a biological variable", "sex difference", 
...
# kw_per_year <- function(keyword="maternal imprint"){
#   
#   x <-   oa_fetch(entity = "works",
#                   title_and_abstract.search = keyword,
#                   verbose=TRUE,
#                   group_by = 'publication_year',
#                   mailto = oa_email())
#   x |>
#     select(publication_year = key, !!keyword := count)
#   
# }
# 
# kw_per_year_sum <- function(keywords=c("maternal imprint", "sex difference")){
#   
#   reduce(
#     map(keywords, kw_per_year),
#     ~left_join(.x, .y, by = "publication_year")
#   )
# }
# 
# keywords_year <- kw_per_year_sum(list_keywords) |> 
#   arrange(publication_year)

# per domain -------------------------------------------------------------------

# kw_per_domain <- function(keyword="maternal imprint"){
#   
#   x <-   oa_fetch(entity = "works",
#                   title_and_abstract.search = keyword,
#                   verbose=TRUE,
...
#                   mailto = oa_email())
#   x |>
#     select(domain = key_display_name, !!keyword := count)
#   
# }
# 
# kw_per_domain_sum <- function(keywords=c("maternal imprint", "sex difference")){
#   
#   reduce(
#     map(keywords, kw_per_domain),
#     ~left_join(.x, .y, by = "domain")
#   )
# }

# keywords_domain <- kw_per_domain_sum(list_keywords)
...
# topics_per_year <- function(topic="t13252"){
#   
#   x <-   oa_fetch(entity = "works",
#                   topics.id = topic,
#                   verbose=TRUE,
...
#                   mailto = oa_email())
#   x |>
#     select(publication_year = key, !!topic := count)
#   
# }
# 
# topics_per_year_sum <- function(topics=c("t13252", "t11077")){
#   
#   reduce(
#     map(topics, topics_per_year),
#     ~left_join(.x, .y, by = "publication_year")
#   )
# }
# 
# topics_year <- topics_per_year_sum(list_topics) |> 
#   arrange(publication_year)

# per domain -------------------------------------------------------------------
# 
# topics_per_domain <- function(topic="t13252"){
#   
#   x <-   oa_fetch(entity = "works",
#                   topics.id = topic,
#                   verbose=TRUE,
...
#                   mailto = oa_email())
#   x |>
#     select(domain = key_display_name, !!topic := count)
#   
# }
# 
# topics_per_domain_sum <- function(topics=c("t13252", "t11077")){
#   
#   reduce(
#     map(topics, topics_per_domain),
#     ~left_join(.x, .y, by = "domain")
#   )
# }
# 
# topics_domain <- topics_per_domain_sum(list_topics)

# Download data for year + domain ----------------------------------------------

# keywords ---------------------------------------------------------------------

# kw_dataset <- function(keyword="maternal imprint"){
#   
#   x <-   oa_fetch(entity = "works",
#                   title_and_abstract.search = keyword,
#                   verbose=TRUE,
#                   mailto = oa_email(),
#                   options = list(
#                     select=c('id','publication_year', 'topics')))
#   x |>
#     mutate(
#       domain = map_chr(topics, ~ {
#         df <- as_tibble(.x)
#         # if there really is an 'i' column, use it; otherwise pick the 1st domain
#         if("i" %in% names(df)) {
#           df %>% filter(i == 1, type == "domain") %>% pull(display_name)
#         } else {NA_character_
#         }
#       }
#       )
...
# }
# 
# kw_dataset_sum <- function(keywords=c("maternal imprint", "sex difference")){
#   
#   reduce(
#     map(keywords, kw_dataset),
#     ~left_join(.x, .y, by = join_by(publication_year,domain))
#   )
# }

#keywords_df <- kw_dataset_sum(list_keywords)
#readr::write_csv(keywords_df, "keywords_df.csv")
...
# topics_dataset <- function(topic="t13252"){
#   
#   x <-   oa_fetch(entity = "works",
#                   topics.id = topic,
#                   verbose=TRUE,
...
# }
# 
# topics_dataset_sum <- function(topics=c("t13252", "t11077")){
#   
#   reduce(
#     map(topics, topics_dataset),
#     ~left_join(.x, .y, by = join_by(publication_year,domain))
#   )
# }

#topics_df <- topics_dataset_sum(list_topics)
#readr::write_csv(topics_df, "topics_df.csv")

topics_df <- read_csv("data/OpenAlex/topics_df.csv")

# Pivot topics table to long format
# topics_df <- topics_df %>%
#   rename(Year = publication_year, Domain = domain) %>%
#   pivot_longer(
#     cols     = -c(Year, Domain),
...
#     t13252 = "Sex and Gender in Healthcare (t13252)",
#     t11077 = "Sex Determination and Differentiation in Organisms (t11077)",
#     t11629 = "Birth, Development, and Health (t11629)",
#     t11389 = "Maternal and fetal healthcare (t11389)"
#   ))
...
# Summarize total counts per year and keyword, omitting NAs in Count
...
keyword_year <- keywords_df %>%
  ...
  mutate(Keyword = case_when(Keyword == "genomic" ~ "genómica",
                             ...
                             Keyword == "maternal imprint" ~ "impresión materna",
